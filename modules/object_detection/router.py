...
async def detect_objects(
    ...
    try:
        ...
        parsed_names = [name.strip() for name in object_names.split(",") if name.strip()]

    if not parsed_names:
        return error(message="list should not be empty", status_code=400, error_code="EMPTY_LIST")
    ...
    if not content:
        return error(message="File content is empty.", status_code=400, error_code="EMPTY_FILE")
